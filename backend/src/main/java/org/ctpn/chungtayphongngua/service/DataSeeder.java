...
public class DataSeeder implements CommandLineRunner {
    ...
    public DataSeeder(UserRepository userRepository, RoleRepository roleRepository, UserRoleRepository userRoleRepository, CourseRepository courseRepository, UserProfileRepository userProfileRepository, AssessmentTypeRepository assessmentTypeRepository, LessonRepository lessonRepository, EnrollmentRepository enrollmentRepository, CategoryRepository categoryRepository, BlogPostRepository blogPostRepository) {
    ...
    private void seedUsersAndProfiles() {
        ...
        User manager = createUser("manager@ctpn.org", "Tran Thi Hanh", "0901234568", LocalDate.of(1988, 8, 20), managerRole);
        User staff = createUser("staff@ctpn.org", "Le Minh Tuan", "0901234569", LocalDate.of(1990, 12, 3), staffRole);

        User consultant1 = createUser("consultant1@ctpn.org", "Bac si Pham Thi Lan", "0901234570", LocalDate.of(1982, 3, 22), consultantRole);
        createConsultantProfile(consultant1, "Bác sĩ chuyên khoa tâm thần...", Arrays.asList("addiction_counseling", "family_therapy"), 12, 4.8, 156, 234);

        User consultant2 = createUser("consultant2@ctpn.org", "Thac si Vu Dinh Nam", "0901234571", LocalDate.of(1979, 11, 18), consultantRole);
        createConsultantProfile(consultant2, "Thạc sĩ Tâm lý học...", Arrays.asList("youth_support", "family_therapy"), 8, 4.9, 89, 145);
    ...
    private void createConsultantProfile(User consultant, String bio, List<String> specializations, int exp, double rating, int reviews, int sessions) {
    ...
    private void seedCoursesAndLessons() {
        ...
        // Course 1
        Course course1 = createCourse("Hiểu biết cơ bản về phòng chống tệ nạn xã hội", "Kiến thức cơ bản về tác hại của ma túy, rượu bia và các chất gây nghiện.", staff, "Giáo dục phòng ngừa", "beginner", 4, 50, 4.5, 123);
        createLesson(course1, "Bài 1: Tổng quan về tệ nạn xã hội", "Giới thiệu về các loại chất gây nghiện phổ biến tại Việt Nam.", 45, 1);
        createLesson(course1, "Bài 2: Tác hại của ma túy", "Phân tích chi tiết các tác hại của ma túy đối với sức khỏe thể chất và tinh thần.", 50, 2);

        // Course 2
        createCourse("Hỗ trợ gia đình có người nghiện chất", "Hướng dẫn gia đình cách nhận biết, ứng phó và hỗ trợ người thân.", consultant1, "Hỗ trợ gia đình", "intermediate", 8, 30, 4.7, 89);

        // Course 3
        createCourse("Kỹ năng sống và phòng chống tệ nạn cho thanh thiếu niên", "Giáo dục kỹ năng sống, nâng cao nhận thức và khả năng từ chối.", consultant2, "Giáo dục thanh thiếu niên", "beginner", 6, 40, 4.8, 156);

        // Course 4: New Drug-Specific Course
        Course course4 = createCourse("Tác hại của Ma túy đá và Heroin", "Phân tích chuyên sâu về tác động của Methamphetamine và Heroin lên não bộ và cơ thể.", consultant1, "Giáo dục chuyên sâu", "intermediate", 5, 40, 4.9, 110);
        createLesson(course4, "Bài 1: Methamphetamine (Ma túy đá)", "Cơ chế hoạt động, tác hại tức thời và lâu dài.", 60, 1);
        createLesson(course4, "Bài 2: Heroin", "Nguồn gốc, cách gây nghiện và các biến chứng nguy hiểm.", 60, 2);

        // Course 5: New Legal Course
        Course course5 = createCourse("Pháp luật Việt Nam về Ma túy", "Tổng quan các quy định pháp luật hiện hành về tàng trữ, mua bán và sử dụng ma túy.", manager, "Pháp luật và Chính sách", "beginner", 3, 100, 4.6, 95);
        createLesson(course5, "Bài 1: Các tội danh liên quan đến ma túy", "Phân tích các điều khoản trong Bộ luật Hình sự.", 45, 1);
        createLesson(course5, "Bài 2: Quy trình xử lý pháp lý", "Các bước xử lý khi bị phát hiện liên quan đến ma túy.", 45, 2);

        // Course 6: New Relapse Prevention Course
        Course course6 = createCourse("Kỹ năng phòng chống tái nghiện", "Cung cấp các công cụ và kỹ năng cần thiết để duy trì sự phục hồi lâu dài.", consultant2, "Phục hồi và Tái hòa nhập", "advanced", 10, 25, 4.9, 80);
        createLesson(course6, "Bài 1: Nhận diện các tác nhân gây tái nghiện", "Học cách xác định các tình huống, cảm xúc và con người có nguy cơ cao.", 75, 1);
        createLesson(course6, "Bài 2: Xây dựng kế hoạch đối phó", "Tạo một kế hoạch hành động cụ thể khi đối mặt với cám dỗ.", 90, 2);
        
        logger.info("Courses and lessons seeded.");
    }

    private Course createCourse(String title, String desc, User inst, String cat, String level, int dur, int max, double rating, int reviews) {
    ...
    private void seedAssessmentTypes() {
        ...
        createAssessmentType("ASSIST", "adult", "WHO Alcohol, Smoking and Substance Involvement Screening Test", "frequency_scoring", assistJson);
    ...
    private void seedBlogContent() {
        ...
        createBlogPost(staff, cat1, "5 dấu hiệu nhận biết con em có nguy cơ sử dụng chất gây nghiện", "Là cha mẹ, việc nhận biết sớm...", "Một bài viết ngắn gọn về các dấu hiệu cảnh báo.");
        createBlogPost(consultant1, cat2, "Cách xây dựng môi trường gia đình tích cực phòng chống tệ nạn", "Gia đình là tuyến phòng thủ đầu tiên...", "Hướng dẫn chi tiết cho các bậc cha mẹ.");
