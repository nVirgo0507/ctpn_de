...
public class BlogService {
    ...
    public Page<Map<String, Object>> getPublishedBlogPosts(Pageable pageable, String category) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            queryBuilder.append("SELECT bp.post_id, bp.title, bp.excerpt, bp.featured_image, ")
                       .append("bp.published_at, bp.views_count, bp.likes_count, ")
                       .append("u.full_name as author_name, c.name as category_name, c.color as category_color ")
            ...
            while (rs.next()) {
                ...
                post.put("viewsCount", rs.getInt("views_count"));
                ...
                post.put("categoryColor", rs.getString("category_color"));
    ...
    public Map<String, Object> getBlogPostById(Long postId, boolean incrementViews) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            String query = "SELECT bp.post_id, bp.title, bp.content, bp.excerpt, bp.featured_image, " +
                          "bp.published_at, bp.views_count, bp.likes_count, bp.created_at, bp.updated_at, " +
            ...
            if (rs.next()) {
                ...
                blogPost.put("viewsCount", rs.getInt("views_count"));
                blogPost.put("likesCount", rs.getInt("likes_count"));
                blogPost.put("createdAt", rs.getTimestamp("created_at"));
                blogPost.put("updatedAt", rs.getTimestamp("updated_at"));
                blogPost.put("authorName", rs.getString("author_name"));
                blogPost.put("categoryName", rs.getString("category_name"));
                blogPost.put("categoryColor", rs.getString("category_color"));
                ...
                if (incrementViews) {
                    ...
                    blogPost.put("viewsCount", rs.getInt("views_count") + 1);
                }
            } else {
    ...
    public Map<String, Object> createBlogPost(Map<String, Object> blogPostData, String userEmail) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            String query = "INSERT INTO ctpn_content.blog_posts " +
                          ...
                          "VALUES (?, ?, ?, ?, ?, ?, 'draft') RETURNING post_id, created_at";
            ...
            if (rs.next()) {
                ...
                createdPost.put("createdAt", rs.getTimestamp("created_at"));
    ...
    public Page<Map<String, Object>> getBlogPostsForManagement(Pageable pageable, String status, String userEmail) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            // Check if user is Manager+ (can see all posts) or Staff (can see only own posts)
            ...
            StringBuilder queryBuilder = new StringBuilder();
            queryBuilder.append("SELECT bp.post_id, bp.title, bp.excerpt, bp.status, bp.created_at, ")
                       .append("bp.published_at, bp.views_count, bp.likes_count, ")
            ...
            queryBuilder.append("ORDER BY bp.created_at DESC LIMIT ? OFFSET ?");
    ...
    public Map<String, Object> approveBlogPost(Long postId, String approverEmail) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            if (rowsUpdated > 0) {
                ...
                throw new RuntimeException("Không thể phê duyệt bài viết. Bài viết có thể đã được xuất bản hoặc không tồn tại.");
            }
    ...
    public Map<String, Object> rejectBlogPost(Long postId, String rejectionReason, String rejectorEmail) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            if (rowsUpdated > 0) {
                ...
                throw new RuntimeException("Không thể từ chối bài viết. Bài viết có thể đã được xử lý hoặc không tồn tại.");
            }
    ...
    public List<Map<String, Object>> getBlogCategories() {
        ...
        try (Connection conn = dataSource.getConnection()) {
            String query = "SELECT category_id, name, description, color, icon " +
